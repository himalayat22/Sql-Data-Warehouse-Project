CREATE OR REPLACE PROCEDURE load_all_silver_table()
LANGUAGE plpgsql
AS $$
BEGIN
...
RAISE NOTICE '=======finished inserting========';
...
sls_prd_key,
sls_cust_id,
case when length(cast(sls_order_dt as text))!=8 then null
else cast(cast(sls_order_dt as varchar(50)) as date)
end as sales_order_dt,
case when length(cast(sls_ship_dt as text))!=8 then null
else cast(cast(sls_ship_dt as varchar(50)) as date)
end as sales_ship_dt,
case when length(cast(sls_due_dt as text))!=8 then null
